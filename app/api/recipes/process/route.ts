...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('⏳ Cache miss. Processing with Gemini:', recipeData.recipeTitle);

    // プロンプトの構築
    const userPrompt = `あなたはレシピ情報を整理し、栄養情報を推定するアシスタントです。必ず有効なJSONのみを返してください。

以下のレシピから、詳しくわかりやすい調理手順、材料と金額、カロリー情報を整理してJSON形式で返してください。
...
- 材料の分量も具体的に（「適量」ではなく「大さじ2」など）`;
    ...
    return NextResponse.json(
      { error: 'レシピの処理に失敗しました', details: (error as any).message },
      { status: 500 } // Geminiはステータスコードを直接持たない場合があるので500固定かエラー型チェックが必要
    );
  }
}
